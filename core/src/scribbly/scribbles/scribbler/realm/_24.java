...
public class _24 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _25());
					} else {
						//reset
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(50, 2, new Vector2(0,-14), 1);
		...
		//platform
		ground = new Polygon(1, 1f, new Vector2(0, -5), 3);
		...
		//center platform1
		ground = new Polygon(1, 1f, new Vector2(-18, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//center platform3
		ground = new Polygon(1, 1f, new Vector2(-8, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//center platform4
		ground = new Polygon(1, 1f, new Vector2(-3, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//center platform5
		ground = new Polygon(1, 1f, new Vector2(2, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//center platform7
		ground = new Polygon(1, 1f, new Vector2(12, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//center platform8
		ground = new Polygon(1, 1f, new Vector2(17, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//shifting_platform
		ground = new Polygon(1, 1f, new Vector2(0, 8), 3);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		//dot
		Particle dot = new Particle(0, -6, 1, 2);
	...
	public void render(float delta) {
		...
		//shifting platform
		if (move_left) {
			if (shifting_platform.getPosition().x > -16) {
				shifting_platform.setLinearVelocity(new Vector2(-2, 0));
			}
			if (shifting_platform.getPosition().x < 16) {
				move_left = false;
				move_right = true;
	...
	private void load_zone() {
		...
		for (Array<Polygon> ap : line_list) {
			...
			ap.get(0).bodyDef.gravityScale = -4f;
			...
			for (int i = 1; i < ap.size-1; i++) {
				...
				ap.get(i).bodyDef.gravityScale = -4f;
	...
	private void spawn_goo() {
		...
		Particle particle = new Particle(mouse.x, mouse.y, 2);
		particle.fixtureDef.filter.categoryBits = 2; //is a
