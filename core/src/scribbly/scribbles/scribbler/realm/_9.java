...
public class _9 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _10());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _9());
					}
				}
		...
		//platform1
		ground = new Polygon(25, 1, new Vector2(-5, 4), 1);
		...
		ground.bodyDef.angle = .5f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//platform2
		ground = new Polygon(25, 1, new Vector2(2, -5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = .5f;
		...
		//dot platform
		ground = new Polygon(1, 1, new Vector2(8, 3), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//dot
		Particle dot = new Particle(8, 5, 1, 1);
