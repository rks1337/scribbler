...
public class _23 implements Screen {
	...
	private String level = "23";

	private Body shifting_platform;
	private Body turning_platform1;
	private Body turning_platform2;
	private boolean move_left = true;
	private boolean move_right;
	private boolean stop_kinematics;
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _24());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _23());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(10, 2, new Vector2(0,-14), 1);
		ground.fixtureDef.filter.categoryBits = 1; //is a
		ground.fixtureDef.filter.maskBits = 4; //collides with
		ground.bodyDef.gravityScale = -1f;
		...
		//platform
		ground = new Polygon(.1f, 2.5f, new Vector2(0, 10), 1);
		...
		//platform2
		ground = new Polygon(1, 1, new Vector2(0, 3f), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 0;
		ground.bodyDef.type = BodyType.KinematicBody;
		turning_platform1 = world.createBody(ground.bodyDef);
		turning_platform1.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//platform2
		ground = new Polygon(1, 1, new Vector2(0, -2.5f), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 0;
		ground.bodyDef.type = BodyType.KinematicBody;
		turning_platform2 = world.createBody(ground.bodyDef);
		turning_platform2.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//platform3
		ground = new Polygon(.1f, 2.5f, new Vector2(0, -9), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base
		ground = new Polygon(1, 1f, new Vector2(-12, 5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base 2
		ground = new Polygon(1, 2, new Vector2(18, -14), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//shifting platform
		ground = new Polygon(1.5f, .1f, new Vector2(16, -5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 0;
		ground.bodyDef.angle = 4;
		ground.bodyDef.type = BodyType.KinematicBody;
		shifting_platform = world.createBody(ground.bodyDef);
		shifting_platform.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//button
		ground = new Polygon(1.5f, .25f, new Vector2(-5, -11.7f), 3);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 2.256f;
		...
		//dot
		Particle dot = new Particle(-12, 6, 1, 1);
		...
		world.setContactListener(new ContactListener() {
			...
			public void postSolve(Contact contact, ContactImpulse impulse) {
				if (contact.getFixtureB().getBody().getGravityScale() == 4.1f &&
						contact.getFixtureA().getBody().getGravityScale() == 2.256f && !complete) {
					stop_kinematics = true;
				}
			}
	...
	public void render(float delta) {
		...
		if (!stop_kinematics) {
			...
			turning_platform1.setAngularVelocity(5f);
			turning_platform2.setAngularVelocity(-5f);

			//shifting platform
			if (move_left) {
				if (shifting_platform.getPosition().x > 11) {
					shifting_platform.setLinearVelocity(new Vector2(-10, -10));
				}
				if (shifting_platform.getPosition().x < 11) {
					move_left = false;
					move_right = true;
			...
			if (move_right) {
				if (shifting_platform.getPosition().x < 18) {
					shifting_platform.setLinearVelocity(new Vector2(10, 10));
				}
				if (shifting_platform.getPosition().x > 18) {
					move_left = true;
			...
			shifting_platform.setLinearVelocity(new Vector2(0, 0));
			turning_platform1.setAngularVelocity(0);
			turning_platform2.setAngularVelocity(0);
		}

		//System.out.println(shifting_platform.getPosition().x);
	...
	private void load_zone() {
		...
		for (Vector2 goo : zone_handler.loaded_goo) {
			Particle particle = new Particle(goo.x, goo.y, 1);
