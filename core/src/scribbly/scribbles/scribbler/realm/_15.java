...
public class _15 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _16());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _15());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(3, 2, new Vector2(-8, -14), 1);
		...
		//diagonal wall 1
		ground = new Polygon(10, .1f, new Vector2(0, 10), 1);
		...
		ground.bodyDef.angle = -.7f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//diagonal wall 2
		ground = new Polygon(10, .1f, new Vector2(15, 5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = .7f;
		...
		//diagonal wall 3
		ground = new Polygon(20, .1f, new Vector2(-10, 7), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -.7f;
		...
		//platform
		ground = new Polygon(1, 1, new Vector2(10, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//dot
		Particle dot = new Particle(10, 7, 1, 1);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(-8, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(-6, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -.1f;
