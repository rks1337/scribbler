...
public class _2 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _3());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _2());
					}
				}
		...
		//platform
		ground = new Polygon(1, 1, new Vector2(-8, 5), 1);
		...
		//platform2
		ground = new Polygon(1, 1, new Vector2(-2, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//platform3
		ground = new Polygon(1, 1, new Vector2(4, -5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//dot
		Particle dot = new Particle(-8, 7, 1, 1);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(9f, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(7, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(11, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -.1f;
