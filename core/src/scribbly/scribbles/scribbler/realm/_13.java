...
public class _13 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _14());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _13());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(40, 2, new Vector2(39.9f, -14), 1);
		...
		//platform
		ground = new Polygon(10, .1f, new Vector2(10, 4), 1);
		...
		//slide
		ground = new Polygon(5, .1f, new Vector2(-4.8f, 2.5f), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = .3f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//wall
		ground = new Polygon(.1f, 4, new Vector2(0, -8f), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
