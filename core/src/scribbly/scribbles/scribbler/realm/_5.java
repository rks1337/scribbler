...
public class _5 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _6());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _5());
					}
				}
		...
		//platform
		ground = new Polygon(1, 1f, new Vector2(-7, 1), 1);
		...
		//lid
		ground = new Polygon(7, .2f, new Vector2(6, -5), 3);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.fixtureDef.density = 1f;
		ground.fixtureDef.friction = 1f;
		ground.fixtureDef.restitution = 0;
		ground.bodyDef.gravityScale = 2f;
		ground.bodyDef.type = BodyType.DynamicBody;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(6, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(4, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
