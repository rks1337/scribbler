...
public class _22 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _22());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(2, 2, new Vector2(0, -14), 1);
		...
		//platform
		ground = new Polygon(1, 1f, new Vector2(0, 5), 1);
		...
		//dot
		Particle dot = new Particle(0, 6, 1, 1);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(0, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(-2, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground.bodyDef.type = BodyType.KinematicBody;
		ground.bodyDef.angularVelocity = 2.5f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		...
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		ground.bodyDef.angularVelocity = -4;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		...
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		ground.bodyDef.angularVelocity = 6;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
