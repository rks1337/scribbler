...
public class _26 implements Screen {
	...
	private String level = "26";

	private Body falling_platform;
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _27());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _26());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(15, 2, new Vector2(-12,-14), 1);
		...
		ground = new Polygon(10, 2, new Vector2(25,-14), 1);
		ground.fixtureDef.filter.categoryBits = 1; //is a
		ground.fixtureDef.filter.maskBits = 4; //collides with
		ground.bodyDef.gravityScale = -1f;
		body = world.createBody(ground.bodyDef);
		...
		ground = new Polygon(6, 2, new Vector2(9f,-14), 1);
		ground.fixtureDef.filter.categoryBits = 1; //is a
		ground.fixtureDef.filter.maskBits = 4; //collides with
		ground.bodyDef.gravityScale = 10f;
		falling_platform = world.createBody(ground.bodyDef);
		falling_platform.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//platform
		ground = new Polygon(.1f, 2.5f, new Vector2(1.5f, 5), 1);
		...
		ground.bodyDef.angle = -4f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		//platform2
		ground = new Polygon(.1f, 2.5f, new Vector2(-4, 2), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = 4f;
		...
		//platform3
		ground = new Polygon(.1f, 2.5f, new Vector2(1.5f, -1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -4f;
		...
		//platform4
		ground = new Polygon(.1f, 2.5f, new Vector2(-4, -4), 3);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		...
		//base
		ground = new Polygon(1, 1f, new Vector2(5, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base2
		ground = new Polygon(1, 1f, new Vector2(8, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base3
		ground = new Polygon(1, 1f, new Vector2(11, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base4
		ground = new Polygon(1, 1f, new Vector2(14, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base5
		ground = new Polygon(1, 1f, new Vector2(17, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//base6
		ground = new Polygon(1, 1f, new Vector2(20, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//dot
		Particle dot = new Particle(5, 7, 1, 1);
		...
		world.setContactListener(new ContactListener() {
			...
			public void endContact(Contact contact) {
				...
				if (contact.getFixtureB().getBody().getGravityScale() == 5f &&
						contact.getFixtureA().getBody().getGravityScale() == 2.256f && !complete) {
					stop_kinematics = false;
				}
			}
	...
	public void render(float delta) {
		...
		if (!stop_kinematics) {
			falling_platform.setType(BodyType.DynamicBody);
		}
