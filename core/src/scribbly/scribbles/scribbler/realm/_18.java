...
public class _18 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _19());
					} else {
						//reset
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(5, 2, new Vector2(8, -14), 1);
		...
		//platform
		ground = new Polygon(1, 1f, new Vector2(-9, 5), 1);
		...
		//dot
		Particle dot = new Particle(-9, 7, 1, 1);
		...
		//spindle1
		ground = new Polygon(2.5f, .1f, new Vector2(-3, 3), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		...
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 2.5f, new Vector2(-3, 3), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		//spindle2
		ground = new Polygon(2.5f, .1f, new Vector2(2, -1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 2.5f, new Vector2(2, -1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(12, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		ground = new Polygon(.1f, 2f, new Vector2(14, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -.1f;
