...
public class _35 implements Screen {
	...
	private String level = "35";

	private Body shifting_platform1;
	private Body shifting_platform2;
	private Body shifting_platform3;
	private Body shifting_platform4;
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _36());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _35());
					}
				}
		...
		ground = new Polygon(.1f, 2.5f, new Vector2(-14, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		//spindle2
		ground = new Polygon(2.5f, .1f, new Vector2(-7, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 2.5f, new Vector2(-7, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		//spindle3
		ground = new Polygon(2.5f, .1f, new Vector2(0, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 2.5f, new Vector2(0, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		//base
		ground = new Polygon(1, 1f, new Vector2(-7, 5), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		...
		//dot
		Particle dot = new Particle(-7, 8, 1, 1);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(7, -12), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 2.256f;
		ground.bodyDef.type = BodyType.KinematicBody;
		shifting_platform1 = world.createBody(ground.bodyDef);
		shifting_platform1.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		//-2, -2
		ground = new Polygon(.1f, 2f, new Vector2(5, -10), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 2.256f;
		ground.bodyDef.angle = .1f;
		ground.bodyDef.type = BodyType.KinematicBody;
		shifting_platform2 = world.createBody(ground.bodyDef);
		shifting_platform2.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		//+4, 0
		ground = new Polygon(.1f, 2f, new Vector2(9, -10), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 2.256f;
		ground.bodyDef.angle = -.1f;
		ground.bodyDef.type = BodyType.KinematicBody;
		shifting_platform3 = world.createBody(ground.bodyDef);
		shifting_platform3.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();

		ground = new Polygon(2, 1, new Vector2(7, -13), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = 2.256f;
		ground.bodyDef.type = BodyType.KinematicBody;
		shifting_platform4 = world.createBody(ground.bodyDef);
		shifting_platform4.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
	...
	public void render(float delta) {
		...
		//shifting platform
		if (move_left) {
			if (shifting_platform1.getPosition().x > -20) {
				shifting_platform1.setLinearVelocity(new Vector2(-10, 0));
			}
			if (shifting_platform1.getPosition().x < -20) {
				move_left = false;
				move_right = true;
			}
			if (shifting_platform2.getPosition().x > -20) {
				shifting_platform2.setLinearVelocity(new Vector2(-10, 0));
			}
			if (shifting_platform2.getPosition().x < -20) {
				move_left = false;
				move_right = true;
			}
			if (shifting_platform3.getPosition().x > -20) {
				shifting_platform3.setLinearVelocity(new Vector2(-10, 0));
			}
			if (shifting_platform3.getPosition().x < -20) {
				move_left = false;
				move_right = true;
			}
			if (shifting_platform4.getPosition().x > -20) {
				shifting_platform4.setLinearVelocity(new Vector2(-10, 0));
			}
			if (shifting_platform4.getPosition().x < -20) {
				move_left = false;
				move_right = true;
