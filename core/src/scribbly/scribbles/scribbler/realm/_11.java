...
public class _11 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _12());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _11());
					}
				}
		...
		//platform left
		ground = new Polygon(15, 1, new Vector2(-20, 1), 1);
		...
		//dot
		Particle dot = new Particle(-10, 5, 1, 1);
		...
		//spindle
		ground = new Polygon(4, .1f, new Vector2(0, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		...
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 4, new Vector2(0, 0), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
