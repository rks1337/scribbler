...
public class _19 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _20());
					} else {
						//reset
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(3, 2, new Vector2(0, -14), 1);
		...
		//dot
		Particle dot = new Particle(0, 7, 1, 1);
		...
		//spindle2
		ground = new Polygon(4, .1f, new Vector2(0,-1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
		ground.bodyDef.angularVelocity = 2;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef).setUserData(ground.polygon_sprite);
		ground.polygon_shape.dispose();
		ground = new Polygon(.1f, 4, new Vector2(0, -1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4;
		ground.bodyDef.type = BodyType.KinematicBody;
