...
public class _7 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _8());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _7());
					}
				}
		...
		//platform2
		//		ground = new Polygon(15, 1, new Vector2(-22, -2), 1);
		//		ground.fixtureDef.filter.categoryBits = 4; //is a
		//		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		//		ground.bodyDef.gravityScale = -4f;
		//		body = world.createBody(ground.bodyDef);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(-16, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(-18, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
