...
public class _4 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _4());
					}
				}
		...
		//platform-3
		ground = new Polygon(1, 1f, new Vector2(-6, -1), 1);
		...
		//platform-2
		ground = new Polygon(1, 1f, new Vector2(-4, 1), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//platform-1
		ground = new Polygon(1, 1f, new Vector2(-2, 3), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
