...
public class _17 implements Screen {
	...
	public void show() {
			...
			public boolean touchDown(float x, float y, int pointer, int button) {
				if (complete && fade == 0.5f) {
					...
					if (take_screenshot) {
						...
						//next
						((Game) Gdx.app.getApplicationListener()).setScreen(new _18());
					} else {
						//reset
						((Game) Gdx.app.getApplicationListener()).setScreen(new _17());
					}
				}
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(3, 2, new Vector2(10,-14), 1);
		...
		//platform
		ground = new Polygon(1, 1, new Vector2(-7, 1), 1);
		...
		//up wall
		ground = new Polygon(.1f, 10, new Vector2(0, 6), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//up platform wall
		ground = new Polygon(3, .1f, new Vector2(0, -4), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//down wall
		ground = new Polygon(.1f, 5, new Vector2(0, -12), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		body = world.createBody(ground.bodyDef);
		...
		//shifting platform
		ground = new Polygon(3, .1f, new Vector2(0, -7), 1);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.fixtureDef.friction = 10;
		...
		//dot
		Particle dot = new Particle(-7, 7, 1, 1);
		...
		//cup
		ground = new Polygon(1.75f, .1f, new Vector2(12f, -11.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.type = BodyType.KinematicBody;
		...
		ground = new Polygon(.1f, 2f, new Vector2(10, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		...
		ground = new Polygon(.1f, 2f, new Vector2(14f, -9.75f), 0);
		ground.fixtureDef.filter.categoryBits = 4; //is a
		ground.fixtureDef.filter.maskBits = 1 | 2 | 4; //collides with
		ground.bodyDef.gravityScale = -4f;
		ground.bodyDef.angle = -.1f;
	...
	public void render(float delta) {
		...
		if (move_left) {
			if (shifting_platform.getPosition().x > -11) {
				shifting_platform.setLinearVelocity(new Vector2(-3, 0));
			}
			if (shifting_platform.getPosition().x < -11) {
				move_left = false;
				move_right = true;
