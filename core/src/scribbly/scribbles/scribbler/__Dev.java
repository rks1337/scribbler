package scribbly.scribbles.scribbler;
...
public class __Dev implements Screen {
	...
	private boolean spawner_switch = false;
	...
	private int remaining_goo;

	Preferences pref = Gdx.app.getPreferences("$420%");
	...
	public void show() {
		...
		//sky box
		Body body;
		Polygon ground = new Polygon(50, 2, new Vector2(0,-14), 3);
		...
		ground = new Polygon(1, 50, new Vector2(-21f, 0), 0);
		ground.fixtureDef.filter.categoryBits = 1; //is a
		ground.fixtureDef.filter.maskBits = 4; //collides with
		ground.bodyDef.gravityScale = -2f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef);
		ground.polygon_shape.dispose();

		ground = new Polygon(1, 50, new Vector2(21f, 0), 0);
		ground.fixtureDef.filter.categoryBits = 1; //is a
		ground.fixtureDef.filter.maskBits = 4; //collides with
		ground.bodyDef.gravityScale = -3f;
		body = world.createBody(ground.bodyDef);
		body.createFixture(ground.fixtureDef);
		ground.polygon_shape.dispose();

		ground = new Polygon(3, .1f, new Vector2(-10, 5), 3);
		...
		Particle dot = new Particle(-10, 6, 1, 1);
		...
		//load zone from json
		zone_handler.load_zone(Gdx.files.internal("json/zone.json"));
		load_zone();
	...
	public void render(float delta) {
		...
		for (Body body : bodies_to_render) {
			if (body.getUserData() instanceof Sprite) {
				...
				//check if goo is drained
				if (body.getGravityScale() == 4.2f) {
		...
		if (Gdx.input.isKeyPressed(Input.Keys.BACK) || Gdx.input.isKeyPressed(Input.Keys.ESCAPE)) {
			((Game) Gdx.app.getApplicationListener()).setScreen(new __Dev());
		}
	}
	...
	private void init_hud() {
		...
		//instructions button
		instruction = new TextButton("", style);
		instruction_input = new InputListener() {
			@Override
		...
		instruction.addListener(instruction_input);
		/////////////////////////////////libby_regular_84/////////////////////////////////
		...
		/////////////////////////////////libby_regular_72_green///////////////////////////
		font = Assets.manager.get("libby_regular_72_green.ttf", BitmapFont.class);
		style.font = font;
		...
		next.addAction(Actions.forever(Actions.sequence(Actions.fadeIn(1f), Actions.fadeOut(1f))));
		/////////////////////////////////libby_regular_72_green///////////////////////////

		/////////////////////////////////libby_regular_72_red/////////////////////////////
		font = Assets.manager.get("libby_regular_72_red.ttf", BitmapFont.class);
		style.font = font;

		//replay button
		replay = new TextButton("replay", style);
		...
		replay.addAction(Actions.forever(Actions.sequence(Actions.fadeIn(1f), Actions.fadeOut(1f))));
		/////////////////////////////////libby_regular_72_red/////////////////////////////
	...
	private void table_animations_and_pass_detection(float delta) {
		...
		if (start_timer) {
			if (time_limit > 0.1f) {
				...
				instruction.removeListener(instruction_input);
				instruction.setText("fail");

				table1.row();
				table1.add(replay).left();
		...
		//win
		if (remaining_goo < 20 && remaining_goo != 0f && !complete) {
			pass = true;
		}
		...
		if (pass) {
			...
			instruction.removeListener(instruction_input);
			instruction.setText("pass");

			table1.row();
			table1.add(next).left();
